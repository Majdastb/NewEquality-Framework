...
createlatticestmt
   : CREATE LATTICE qualified_name (ON ATTRIBUTE qualified_name)? USING LABELS labellist
    ORDERED AS orderedstmt ;

labellist
   : OPEN_PAREN LAB (COMMA LAB)* CLOSE_PAREN
   ;

orderedstmt
   : OPEN_PAREN orderedlist CLOSE_PAREN
   ;

orderedlist
   : TOP LAB COMMA BOTTOM LAB COMMA ORDER labelordering
   ;

labelordering
   : (labelpair (COMMA labelpair)*)?
   ;
...
alter_table_cmd
   ...
   | ADD_P LATTICE qualified_name ON ATTRIBUTE attribute_list
   | ADD_P LATTICE L3U ON ATTRIBUTE attribute_list
   | ADD_P LATTICE L3S ON ATTRIBUTE attribute_list
   | ADD_P LATTICE L4 ON ATTRIBUTE attribute_list
   | ADD_P COMPARABILITY qualified_name ON ATTRIBUTE attribute_list
   | ADD_P COMPARABILITY C3U ON ATTRIBUTE attribute_list
   | ADD_P COMPARABILITY C3S ON ATTRIBUTE attribute_list
   | ADD_P COMPARABILITY C4 ON ATTRIBUTE attribute_list
...
createrealitystmt
   : CREATE REALITY qualified_name ON TABLE qualified_name AS realitylist
    ;

 realitylist
    : OPEN_PAREN realityelement (COMMA realityelement)* CLOSE_PAREN
    ;

 realityelement
    : LAB ON ATTRIBUTE qualified_name
    | TOP OTHERWISE
    ;
...
createcompstmt
   : CREATE COMPARABILITY qualified_name
     ON LATTICE qualified_name labellist AS
     (CASE LAB COLON a_expr)*
     OTHERWISE LAB
   ;
